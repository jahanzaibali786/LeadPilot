            ...
            <div class="col-lg-8">
                <section class="panel h-100">
                    <div class="panel-body">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div>
                                ...
                                <span id="live-indicator"
                                    class="badge bg-info-subtle text-info-emphasis ms-2 {{ in_array($campaign->status, ['pending', 'running']) ? '' : 'd-none' }}">
                        ...
                        <div class="d-flex text-center justify-content-between flex-nowrap">
                            @foreach ([['found', 'Found', $campaign->total_found], ['saved', 'Saved', $campaign->total_saved], ['skipped', 'Skipped by filters', max(0, $campaign->total_found - $campaign->total_saved - $campaign->duplicates_removed)], ['duplicates', 'Duplicates', $campaign->duplicates_removed], ['hot', 'Hot', $campaign->hot_leads], ['warm', 'Warm', $campaign->warm_leads], ['requests', 'API calls', $apiRequests], ['failed', 'Failed calls', $campaign->failed_requests]] as [$key, $label, $value])
                                <div class="flex-fill">
                                    <strong id="metric-{{ $key }}" class="h4 d-block animated-counter"
                                        data-value="{{ $value }}">
                                        {{ $value }}
                                    </strong>
                        ...
                        <div id="campaign-error"
                            class="alert alert-danger mt-4 mb-0 {{ $campaign->failure_reason ? '' : 'd-none' }}">
            ...
            <div class="col-lg-4">
                <section class="panel h-100">
                    <div class="panel-body">
                        ...
                        <strong>{{ $campaign->business_category }} in {{ $campaign->city }},
                            {{ $campaign->country }}</strong>
                        @if ($campaign->latitude !== null && $campaign->longitude !== null)
                            <div class="muted mt-1">{{ number_format($campaign->radius_meters / 1000, 1) }} km map radius
                            </div>
                        @endif
                        <hr>
                        <p class="muted mb-1">Qualification</p>
                        <span class="small">Rating {{ $campaign->minimum_rating }}+ · Reviews
                            {{ $campaign->minimum_reviews }}+</span>

                        <div class="d-flex flex-wrap gap-2 mt-4">
                            <div id="active-campaign-actions"
                                class="d-flex gap-2 {{ in_array($campaign->status, ['pending', 'running']) ? '' : 'd-none' }}">
                                <a class="btn btn-outline-info disabled" aria-disabled="true"><span
                                        class="spinner-border spinner-border-sm me-1"></span> Running</a>
                                <form method="post" action="{{ route('campaigns.cancel', $campaign) }}">
                                    @csrf
                                    <button class="btn btn-outline-danger">Cancel</button>
                                </form>
                            </div>
                            <div id="idle-campaign-actions"
                                class="{{ in_array($campaign->status, ['pending', 'running']) ? 'd-none' : '' }}">
                                <form id="run-campaign-form" method="post"
                                    action="{{ route('campaigns.run', $campaign) }}">
                                    @csrf
                            ...
                            @unless(in_array($campaign->status, ['pending', 'running'], true))
                                <a href="{{ route('campaigns.edit', $campaign) }}" class="btn btn-outline-secondary"><i class="bi bi-pencil"></i> Edit</a>
                            @endunless

                            <a href="{{ route('leads.index', ['campaign_id' => $campaign->id]) }}"
                                class="btn btn-outline-secondary">View leads</a>
                        </div>
                    </div>
                </section>
            </div>
        ...
        <section class="panel">
            <div class="panel-head">
                <div>
                    ...
                    <span class="muted">New Google Places leads appear here while the campaign is running.</span>
                </div>
                <a href="{{ route('exports.leads', ['campaign_id' => $campaign->id]) }}"
            ...
            <div class="table-responsive">
                <table class="table mb-0">
                    ...
                    <tbody id="collected-leads-body">
                        @forelse($leads as $lead)
                            <tr>
                                <td><a class="business-name"
                                        href="{{ route('leads.show', $lead) }}">{{ $lead->business_name }}</a>
                                    <div class="muted">{{ $lead->business_category }}</div>
                                </td>
                                <td>{{ $lead->formatted_phone }}</td>
                                <td>★ {{ $lead->rating }} ({{ $lead->total_reviews }})</td>
                                <td>@include('partials.lead-website-link', ['lead' => $lead])</td>
                                <td><span class="badge quality-{{ $lead->lead_quality }}">{{ $lead->lead_score }} ·
                                        {{ $lead->lead_quality }}</span></td>
                                <td>{{ $lead->board_status }}</td>
                            </tr>
                        @empty
                            <tr id="empty-leads-row">
                                <td colspan="6" class="text-center py-5 text-secondary">Run this campaign to collect
                                    compliant public business data.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
@push('scripts')
    <script>
        (() => {
            ...
            const renderLiveLeads = (leads) => {
                if (!leads.length) return;
                const body = document.getElementById('collected-leads-body');
                if (!body) return;
                ...
                leads.forEach((lead) => {
                    ...
                    businessLink.className = 'business-name';
                    businessLink.href = lead.url;
                    businessLink.textContent = lead.business_name;
                    const category = document.createElement('div');
                    category.className = 'muted';
                    category.textContent = lead.business_category || '';
                    businessCell.append(businessLink, category);

                    const phoneCell = document.createElement('td');
                    phoneCell.textContent = lead.formatted_phone || '-';
                    const ratingCell = document.createElement('td');
                    ratingCell.textContent = lead.rating ? `? ${lead.rating} (${lead.total_reviews || 0})` :
                        '-';
                    ...
                    if (lead.has_website && lead.website) {
                        ...
                        badge.className = lead.website_status === 'Social Profile Only'
                            ? 'badge bg-info-subtle text-info-emphasis'
                            : 'badge bg-danger-subtle text-danger';
                        badge.textContent = lead.website_status === 'Social Profile Only' ? 'Social profile only' : 'No website';
                        websiteCell.appendChild(badge);
                    }
                    const scoreCell = document.createElement('td');
                    const scoreBadge = document.createElement('span');
                    scoreBadge.className = `badge quality-${lead.lead_quality || 'Cold'}`;
                    scoreBadge.textContent = `${lead.lead_score || 0} � ${lead.lead_quality || 'Cold'}`;
                    scoreCell.appendChild(scoreBadge);
                    const statusCell = document.createElement('td');
                    statusCell.textContent = lead.board_status || 'New';
                    row.append(businessCell, phoneCell, ratingCell, websiteCell, scoreCell, statusCell);
            ...
            const applyStatus = (data) => {
                ...
                animateCounter('metric-skipped', data.skipped_filters);
                ...
                animateCounter('metric-requests', data.api_requests);
                animateCounter('metric-failed', data.failed_requests);
                renderLiveLeads(data.recent_leads || []);
                ...
                const messages = {
                    ...
                    completed: data.total_saved < data.total_found
                        ? `Campaign completed: ${data.total_saved} of ${data.total_found} found listings matched all qualification filters.`
                        : 'Campaign completed successfully.',
